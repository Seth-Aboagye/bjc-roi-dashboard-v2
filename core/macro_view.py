...
# Reuse shared chart functions from charts.py (you already added these)
from .charts import (
...
def macro_interpretation(model: dict, assumptions: dict) -> str:
    """
    Produces a clear executive-style narrative based on the macro model outputs.
    model keys expected: forecast_df, kpis, budget_df, recommendations
    """
    k = model["kpis"]
    ...
    # Assumptions summary
    retention = assumptions.get("Retention", None)
    margin = assumptions.get("Margin", None)
    ...
    if improving:
        lines.append("ROI **improves over time**, suggesting retained/repeat value and scaling effects outweigh modeled cost growth.")
    elif declining:
        lines.append("ROI **declines over time**, suggesting modeled cost growth/margin pressure may be outpacing retained revenue impact.")
    else:
        lines.append("ROI appears **relatively stable across the 3 years** under the current assumptions.")

    # Assumptions interpretation
    a_bits = []
    if retention is not None:
        a_bits.append(f"Retention is set to **{retention:.0%}**")
    if margin is not None:
        a_bits.append(f"Development margin is **{margin:.0%}**")
    if cost_growth is not None:
        a_bits.append(f"Cost growth is **{cost_growth:.0%}** per year")
    if revenue_shock is not None and revenue_shock != 0:
        a_bits.append(f"Revenue shock is **{revenue_shock:+.0%}**")
    if cost_shock is not None and cost_shock != 0:
    ...
    # Add a final “so what”
    if roi_mult < 1.0:
        lines.append("Overall, this scenario indicates costs exceed forecasted returns; BJC may need to prioritize higher-yield fundraising (e.g., major donors) and tighten cost assumptions.")
    elif roi_mult < 2.0:
        lines.append("Overall, this scenario supports a positive but moderate return; improving retention and cost efficiency would meaningfully strengthen mission resources.")
    else:
        lines.append("Overall, this scenario supports strong returns; the priority becomes scaling what works while monitoring cost growth and retention sensitivity.")
...
def macro_view():
    ...
    method = colC.selectbox(
        ...
        ["remaining", "prior"],
        index=0,
        help="remaining = 60% of remaining; prior = 60% of prior year"
    )
    ...
    k1.metric("Total Revenue (3yr)", f"${k['Total Revenue (3yr)']:,.0f}")
    k2.metric("Total Cost (3yr)", f"${k['Total Cost (3yr)']:,.0f}")
    k3.metric("Total Net (3yr)", f"${k['Total Net (3yr)']:,.0f}")
    ...
    # assumptions dict for interpretation + reports
    assumptions = {
    ...
    with t2:
        ...
        narrative = macro_interpretation(model, assumptions)
